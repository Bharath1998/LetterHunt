    ...
    void OnCollisionEnter2D(Collision2D collision){
    	if (collision.gameObject.name == "Player"){
	        ...
	        for (int i =0; i < targetWord.Length; i++){
	        	letters = GameObject.Find("yellow_a_b_" + targetWord[i].ToString().ToLower() + "(Clone)");
	        	if (letters){
	        		print("NOTNULL");
	        		Vector3 oldPosition = letters.transform.position;
		            Vector3 oldscale = letters.transform.localScale;
		            Destroy(letters);
	        		highlightedLetter = Resources.Load("GreenLetters/a/green_a_b_" + char.ToLower(targetWord[i])) as GameObject;
	                GameObject spawnedHighlightedLetter = Instantiate(highlightedLetter);
	                ...
	                dullLetter = Resources.Load("YellowLetters/a/yellow_a_b_" + char.ToLower(targetWord[i])) as GameObject;
